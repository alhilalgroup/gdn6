jQuery(document).ready(function ($) {
    ...
    $.getJSON('http://cdn.gdnonline.com/storydetail/' + querystring('id') + '/?jsoncallback=?', function (data) {
        ...
        $.each(data, function (key, value) {
            ...
            if (data[key].custom_story_type == "Multipart Story") {
                window.location.href = "multidetails.html?id=" + querystring('id');
            }

            if (data[key].custom_story_type == "VideoImageContent") {
                window.location.href = "vicontentdetails.html?id=" + querystring('id');
            }
            ...
            if (cupageno <= 25) {
                ...
                pagecontent += "<div class='bar'></div><div class='homecategory' style='margin-top:-32px'>" + data[key].category.replace('Local','Bahrain').replace('International','World') + "</div>";
                ...
                if (data[key].imagefilename != '') {
                    if (data[key].custom_story_type != "VideoImageContent") {
                        if (screen.width < 768) {
                            ...
                            pagecontent += "<br style='clear:both'><img style='width:100%;' onerror=this.style.display = 'none' src='http://www.gdnonline.com/gdnimages/" + data[key].mime_type_source + "'>";
                            pagecontent += "<div style='font-size: 12px; text-align: center; color: #666'>" + data[key].caption_text + "</div>";
                ...
                pagecontent += "<div class='roboto dettext' >" + data[key].Article + "</div>";
                pagecontent += "</div>";
                pagecontent += "</div>";
                pagecontent += "</li>";
